...
dat %>% group_by(simNo, noCpgs, method) %>% 
  summarise(pSig = sum(pVals < 0.05)/length(pVals)) -> sigDat
...
dat %>% filter(simNo %in% s) %>%
  arrange(simNo, noCpgs, method, pVals) %>%
  group_by(simNo, noCpgs, method) %>%
  mutate(exp = 1:n()/n()) -> subDat

p <- ggplot(subDat, aes(x=-log10(exp), y=-log10(pVals), color=method)) + 
