...
ggplot(cpgEg, aes(x=Var1, y=Freq)) +
  ...
  geom_text(aes(x=med), label=glue("median = {med}"), y=750, colour="red", 
...
egCpg$Split <- ifelse(egCpg$Freq > 2500, "A", 
                      ifelse(egCpg$Freq < 2500 & egCpg$Freq > 65,"B","C"))
egCpg
```
...
cpgEgGo <- reshape2::melt(missMethyl:::.getGO()$idList)
# need to redo this part to generate subsequent values properly
head(cpgEgGo)
```
...
```{r}
load(here("output/nullSimEPIC.100.long.RData"))
```
