...
The RNAseq dataset used to generate the "truth" gene sets is avoilable from GEO at [GSE107011](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE107011) or SRA at [SRP100803](https://www.ncbi.nlm.nih.gov/sra?term=SRP100803).

The RNAseq data was quasi-mapped and quantified using Salmon (v1.3.0) with the [hg19_cdna](http://refgenomes.databio.org/v2/asset/hg19_cdna/fasta/archive?tag=default) human transcriptome downloaded from [refgenie](http://refgenomes.databio.org/index). The code used to perform the Salmon quasi-mapping and quantification can be found in `code/salmon-quant.sh`. 
...
saveRDS(kegg, here(glue("data/cache-rnaseq/RNAseq-SRP100803-KEGG.rds")))
```
