...
We are using publicly available EPIC data [GSE135446](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE135446) generated from generated from normal and fibroid uterine tissue. The data is normalised and filtered (bad probes, multi-mapping probes, SNP probes, sex chromosomes).

```{r, message=FALSE, warning=FALSE}
# load data
dataFile <- here("data/datasets/GSE135446-data.RData")
...
if(file.exists(dataFile)){
  ...
  dir <- here("data/datasets/GSE135446")
...
dat <- tibble::tibble(mean = colMeans(detP), group = targets$Type, 
...
dat$group <- rep(targets$Type, each = nrow(bVals))

p <- ggplot(dat, aes(x = beta, colour = group, group = ID)) + 
    geom_density(size=0.5, alpha=0.5) +
    labs(color = "Sample", x = "Beta value", y = "Density")
...
for(i in 1:length(dims)){
    ...
    dat <- data.frame(x = tmp$x, y = tmp$y, group = targets$Type)
    p[[i]] <- ggplot(dat, aes(x = x, y = y, colour = group)) +
        geom_point() +
        labs(colour = "Sample", x = glue("PC {tmp$dim.plot[1]}"),
...
design <- model.matrix(~0+targets$Type+targets$Individual)
colnames(design) <- c(levels(factor(targets$Type)), 
                      levels(factor(targets$Individual))[-1])

fit <- lmFit(mVals, design)
cont.matrix <- makeContrasts(NormalVCervix=Normal-Cervix,
                             NormalVFibroid=Normal-Fibroid,
...
de <- readRDS("/oshlack_lab/jovana.maksimovic/research/methyl-geneset-testing/data/datasets/SRP217468/DEgenes.rds")

flat <- missMethyl:::.getFlatAnnotation("EPIC")
flat %>% inner_join(as.data.frame(de$genes) %>% 
                        mutate(entrezid = as.character(entrezid)),
                    by = c("entrezid" = "entrezid")) %>%
    inner_join(rownames_to_column(as.data.frame(tfit$coefficients), 
                                  var = "cpg")) %>%
    inner_join(rownames_to_column(as.data.frame(de$coefficients), 
                                  var = "gene_id"), 
               by = c("gene_id" = "gene_id"))-> foo

dea <- topTable(de, coef = "NormalVCervix", p.value = 0.05, num=Inf)$entrezid
deb <- topTable(de, coef = "NormalVFibroid", p.value = 0.05, num=Inf)$entrezid
dec <- topTable(de, coef = "CervixVFibroid", p.value = 0.05, num=Inf)$entrezid

dma <- rownames(topTreat(tfit, coef = "NormalVCervix", p.value = 0.05, num=Inf))
dmb <- rownames(topTreat(tfit, coef = "NormalVFibroid", p.value = 0.05, num=Inf))
dmc <- rownames(topTreat(tfit, coef = "CervixVFibroid", p.value = 0.05, num=Inf))

a <- ifelse(foo$entrezid %in% dea & foo$cpg %in% dma, "red",
            ifelse(foo$entrezid %in% dea, "blue",
                   ifelse(foo$cpg %in% dma, "green", "grey")))
b <- ifelse(foo$entrezid %in% deb & foo$cpg %in% dmb, "red",
            ifelse(foo$entrezid %in% deb, "blue",
                   ifelse(foo$cpg %in% dmb, "green", "grey")))
c <- ifelse(foo$entrezid %in% dec & foo$cpg %in% dmc, "red",
            ifelse(foo$entrezid %in% dec, "blue",
                   ifelse(foo$cpg %in% dmc, "green", "grey")))

par(mfrow =c(3,1))
plot(foo$NormalVCervix.x, foo$NormalVCervix.y, pch = ".", col=a,ylim = c(-6,6), 
     xlim=c(-5,5))

text(x = -4, y =4, 
     labels = paste0("Spearman: ", round(cor(foo$NormalVCervix.x, 
                                            foo$NormalVCervix.y, 
                                            method = "spearman"), 2)))
plot(foo$NormalVFibroid.x, foo$NormalVFibroid.y, pch = ".", col=b,ylim = c(-6,6), xlim=c(-5,5))
text(x = -4, y =4, 
     labels = paste0("Spearman: ", round(cor(foo$NormalVFibroid.x, foo$NormalVFibroid.y, 
                                            method = "spearman"), 2)))
plot(foo$CervixVFibroid.x, foo$CervixVFibroid.y, pch = ".", col=c,ylim = c(-6,6), xlim=c(-5,5))
text(x = -4, y =4, 
     labels = paste0("Spearman: ", round(cor(foo$CervixVFibroid.x, foo$CervixVFibroid.y, 
                                            method = "spearman"), 2)))
```

```{r}
bDat <- getBiasDat(rownames(topTreat(tfit, coef = 2, num = 5000, 
                                     p.value = 0.05)), 
                   array.type = "EPIC")
...
outFile <- here("data/cache-intermediates/fibroids.contrasts.rds")
...
```{r}
inFiles <- list.files(here("output/compare-methods/FIBROID"), pattern = "rds", 
                      full.names = TRUE)
...
```{r}
rnaseqGO <- readRDS(here("data/cache-rnaseq/RNAseq-SRP217468-GO.rds"))
...
```{r}
rnaseqKEGG <- readRDS(here("data/cache-rnaseq/RNAseq-SRP217468-KEGG.rds"))
...
```{r}
rnaseqBROAD <- readRDS(here("data/cache-rnaseq/RNAseq-SRP217468-BROAD-GSA.rds"))
...
for(i in 1:length(p)){
    ...
    tmp %>% mutate(rna = ID %in% topBROAD$ID[topBROAD$contrast %in% cont],
                   col = ifelse(rna == 1, "RNAseq", "None")) %>%
        mutate(col = factor(col, levels = c("RNAseq", "None"))) -> tmp
