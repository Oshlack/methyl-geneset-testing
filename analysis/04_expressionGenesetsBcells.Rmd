...
```{r}
oligo::boxplot(raw_data, target = "core", xaxt = "none",
               main = "Boxplot of log2-intensitites for the raw data")
axis(1, at = 1:nrow(targets), labels = targets$ID, las = 2)
```
...
Apply the full RMA algorithm to our data in order to background-correct, normalize and summarize
...
We now filter out all transcripts that do not have intensities greater than the threshold in at least as many arrays as the smallest experimental group which we define as `min(no_of_samples)`.
...
Many transcript-cluster identifiers will map to multiple Entrez IDs, i.e.  they can’t be unambigously assigned.
...
The background corrected and RMA normalised data...
...
pval <- 0.01
fitSum <- summary(decideTests(fit2, p.value = pval))
fitSum
```
