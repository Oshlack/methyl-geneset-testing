...
mdat %>% inner_join(flatAnn) %>%
    dplyr::select(-group, -symbol, -alias) -> mdat
mdat <- reshape2::melt(mdat, value.name = "coefficient") %>%
    dplyr::select(-cpg) %>%
    group_by(variable, entrezid) %>%
    summarise(coef = median(coefficient)) -> mdat

mdat %>% inner_join(rdat, by = c("entrezid", "variable")) -> dat

ggplot(dat, aes(x = coef, y = coefficient)) +
    geom_point(alpha = 0.5, size = 0.5) +
    facet_wrap(vars(variable), ncol = 2) +
    labs(x = "Median delta M per gene", y = "Log FC")
...
As the ebGSEA method from the ChAMP package only allows testing of their in-built database of Broad Institute gene sets, we will compare to other methods by testing these same gene sets. We are using the top 100 ranked gene sets as identified by `camera` analysis of the corresponding RNAseq data as "truth".
...
Using the top 100 ranked gene sets as identified by Belinda's `gsaseq` analysis of the corresponding RNAseq data as "truth".
