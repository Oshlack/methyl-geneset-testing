...
```{r}
bDat <- getBiasDat(rownames(topTreat(tfit, coef = "CD4vCD8", num = 5000)), 
                   array.type = "EPIC")
...
```
```{r}
fig <- here("output/figures/SFig-X1.rds")
saveRDS(p, fig, compress = FALSE)
```
...
```{r}
fig <- here("output/figures/SFig-X2.rds")
saveRDS(p, fig, compress = FALSE)
```
...
for(i in 1:length(p)){
    ...
    tmp %>% mutate(rna = ID %in% topBROAD$ID[topGOSets$contrast %in% cont],
                   col = ifelse(rna == 1, "RNAseq", "None")) -> tmp
